...
import { Plus, Edit, Power, Upload, X, CheckCircle, AlertCircle, MapPin, Calendar } from 'lucide-react';
...
const MultiYearCandidateManager = () => {
  ...
  const openDialog = (candidato?: Candidato) => {
    if (candidato) {
      ...
      candidato.anos_data?.forEach(yearData => {
        yearMap[yearData.ano] = {
          ano: yearData.ano,
          secaoFile: null,
          bairroFile: null
        };
      });
      setYearDataMap(yearMap);
    } else {
      resetForm();
    }
    setDialogOpen(true);
  ...
  const handleYearSelection = (year: number, checked: boolean) => {
    if (checked) {
      ...
      setYearDataMap(prev => ({
        ...prev,
        [year]: { ano: year, secaoFile: null, bairroFile: null }
      }));
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!editingId && selectedYears.length === 0) {
      toast({
        title: "Erro",
        description: "Selecione pelo menos um ano para o candidato",
        variant: "destructive"
      });
      return;
    }

    // Verificar se todos os anos selecionados têm arquivos (para novos candidatos)
    if (!editingId) {
      const missingFiles = selectedYears.some(year => {
        const yearData = yearDataMap[year];
        return !yearData?.secaoFile || !yearData?.bairroFile;
      });

      if (missingFiles) {
        toast({
          title: "Erro",
          description: "Todos os anos selecionados devem ter os dois arquivos CSV",
          variant: "destructive"
        });
        return;
      }
    ...
    try {
      ...
      // Processar anos e arquivos CSV
      for (const year of selectedYears) {
        const yearData = yearDataMap[year];
        
        // Salvar registro do ano
        ...
        // Processar arquivos CSV se fornecidos
        if (yearData?.secaoFile && yearData?.bairroFile) {
          // Limpar dados existentes para este ano
          await Promise.all([
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Selecione os anos para os quais você possui dados eleitorais do candidato.
                  </p>
